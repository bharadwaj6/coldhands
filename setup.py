...
def update_config(libdir, values = {}):
    ...
    filename = os.path.join(libdir, 'mybrowser_lib/mybrowserconfig.py')
    oldvalues = {}
    try:
        fin = file(filename, 'r')
...
def move_desktop_file(root, target_data, prefix):
    ...
    old_desktop_path = os.path.normpath(root + target_data +
                                        '/share/applications')
    old_desktop_file = old_desktop_path + '/mybrowser.desktop'
    desktop_path = os.path.normpath(root + prefix + '/share/applications')
    desktop_file = desktop_path + '/mybrowser.desktop'
    ...
    if not os.path.exists(old_desktop_file):
        ...
        desktop_file = desktop_path + '/extras-mybrowser.desktop'
        try:
...
def update_desktop_file(filename, target_pkgdata, target_scripts):
    ...
    try:
        ...
        for line in fin:
            if 'Icon=' in line:
                line = "Icon=%s\n" % (target_pkgdata + 'media/mybrowser.svg')
            elif 'Exec=' in line:
                cmd = line.split("=")[1].split(None, 1)
                line = "Exec=%s" % (target_scripts + 'mybrowser')
                if len(cmd) > 1:
...
class InstallAndUpdateDataDirectory(DistUtilsExtra.auto.install_auto):
    def run(self):
        ...
        target_pkgdata = target_data + 'share/mybrowser/'
        target_scripts = '/' + os.path.relpath(self.install_scripts, self.root) + '/'

        values = {'__mybrowser_data_directory__': "'%s'" % (target_pkgdata),
